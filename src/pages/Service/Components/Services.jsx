...
const Services = () => {
  ...
  useEffect(() => {
    if (location.hash) {
      ...
      if (element) {
        setTimeout(() => {
          const yOffset = -210; // adjust if you have a fixed header
          const y =
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {additionalServices.map((service, index) => (
              <motion.div
                ...
                key={index}
                className={`p-6 rounded-xl shadow-lg border border-white/10 ${service.bgColor} hover:scale-[1.02] transition-transform duration-300`}
              >
                <div
                  className={`w-12 h-12 mb-4 flex items-center justify-center rounded-full bg-gradient-to-br ${service.gradient}`}
                >
